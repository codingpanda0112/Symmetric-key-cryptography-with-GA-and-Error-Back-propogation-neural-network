...
def activate(weights, inputs):
	...
	for i in range(len(weights)-1):
		activation += weights[i] * inputs[i]	#wieght * input + bias
...
def forward_propagate(network, row):
	inputs = row
	print "row in forward forward_propagate=",row

	for layer in network:
		new_inputs = []
		for neuron in layer:
			#print "neuron weights=",neuron['weights']
			activation = activate(neuron['weights'], inputs) #to calculate wieght * input + bias
			#print "activation=",activation
			neuron['output'] = transfer(activation)	#activation function
			print neuron
			new_inputs.append(neuron['output'])	#each layer's output
			#print "new_inputs=",new_inputs
			
		inputs = new_inputs	#inputs keep changing at first for the HL and then for the OL
...
def train_network(network,train_set,n_outputs,l_rate):
	...
	for row in train_set:
			...
			print "-------------------------------------------------------------"
			print("outputs of the output layer=",outputs)
...
def readfile():
	...
	print "no of inputs=",n_inputs,"no of outputs=",n_outputs

	rows=expected(rows,n_inputs)
	print("expected rows=",rows)
	rows=str_to_float(rows,n_inputs)
	print("change str to float=",rows)
	network = initialize_network(n_inputs, 2, n_outputs)
	print "weights for hidden layer and output layer="
	for layer in network:
		print(layer)
